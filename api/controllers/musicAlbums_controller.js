...
module.exports.getInfoByMuscian = async (req, res) => {
  try {
...
module.exports.getInfoByMusicAlbum = async (req, res) => {
  try {
    console.log(req.query)
    var musicAlbumId = req.query.musicAlbumId.split(',').map(info=>ObjectId(info));

    console.log("musicAlbumId",musicAlbumId);
  
    var getMusicanData = await musicAlbumsModel.aggregate([
        {$match:{
            "_id":{
                $in:musicAlbumId
        }
...
module.exports.create = async (req, res) => {
  try {
    ...
    var { albumName, dateOfRelease, genre, price, description, musicians } =
      req.body;
...
module.exports.update = async (req, res) => {
  try {
    ...
    var updatedData = await musicAlbumsModel.updateOne(
      { _id: musicAlblumId },
      { $set: req.body }
    );
    ...
    if (updatedData.nModified == 1) {
      ...
      res.json({
        status: false,
        message: "Music Ablum Information Not NUpdated",
      });
    }
  } catch (e) {
    res.json({ message: "something went wrong", status: false });
